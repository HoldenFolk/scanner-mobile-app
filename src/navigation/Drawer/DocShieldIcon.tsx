...
interface DocShieldIconProps extends SvgProps {
	size?: number;
}
...
const StyledSvg = styled(Svg)<{ size: number }>`
	...
	margin-left: 4px;
	width: ${({ size }) => size}px;
	height: ${({ size }) => (size / 144) * 128}px;
	fill: ${({ theme }) => theme.colors.secondary};
`;

export function DocShieldIcon({ size = 42, ...optionals }: DocShieldIconProps) {
	return (
		<StyledSvg viewBox="0 0 153 136" size={size} {...optionals}>
